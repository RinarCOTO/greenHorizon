...
export default function StatsSection() {
  ...
  return (
    <section ref={sectionRef} className="py-16 sm:py-20 lg:py-28 px-5 sm:px-6 lg:px-10 bg-gray-50">
      <div className="max-w-7xl mx-auto">

        {/* Header */}
        <div className="grid lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12 items-end mb-10 sm:mb-14 lg:mb-16">
          <div className={visible ? "animate-fade-up delay-100" : "opacity-0"}>
            ...
            <h2 className="font-extrabold text-[clamp(1.75rem,4vw,3.2rem)] text-gray-900 leading-tight">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 lg:gap-6">
          {stats.map((stat, i) => (
            <div
              key={stat.value}
              className={`group relative bg-white rounded-3xl p-6 sm:p-8 border border-gray-100 hover:-translate-y-2 hover:shadow-[0_24px_64px_rgba(38,159,98,0.12)] transition-all duration-300 overflow-hidden ${visible ? `animate-fade-up ${cardDelays[i]}` : "opacity-0"}`}
            >
              <div
              ...
              <div className="relative">
                <p className="font-extrabold text-3xl sm:text-4xl leading-none mb-1 gradient-text">
                  {stat.value}
                </p>
                <p className="text-xs font-semibold uppercase tracking-widest text-gray-400 mb-3 sm:mb-4">{stat.label}</p>
